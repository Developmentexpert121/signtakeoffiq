...
/**
 * Custom-metadata key under which the ACL policy JSON is stored on each object.
 * Exported so the S3 backend can map it to/from an S3-safe user-metadata key
 * (S3 metadata keys cannot contain ':' or upper-case, so it is stored there as
 * `aclpolicy` and re-exposed under this key by `S3File.getMetadata()`).
 */
